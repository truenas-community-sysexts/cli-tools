...
BEGIN/END release-selection sentinels is extracted verbatim and run with
canned GitHub releases JSON on stdin, exactly how the scripts run it; the
shell functions are run under bash with stubbed commands."""
import json
import os
import subprocess
import tempfile
...
class SharedCopies(unittest.TestCase):
    def test_block_is_identical_in_get_sh_install_sh_and_uninstall_sh(self):
        ...
        self.assertEqual(shared_block(GET_SH), shared_block(UNINSTALL_SH))

    def test_nothing_is_fetched_from_githubs_latest(self):
        # Latest is cosmetic now: it follows the newest release approved for
        # a stable train, which need not be approved for this box's train.
        for path in (GET_SH, INSTALL_SH, UNINSTALL_SH):
            self.assertNotIn("releases/latest", path.read_text(), path.name)


class Approval(unittest.TestCase):
    def test_marker_for_the_train_is_approved(self):
        rels = [release(tag(12), trains=["26"]), release(R11)]
        p = run_selection(rels, *BETA)
        self.assertEqual(p.returncode, 0, p.stderr)
        self.assertEqual(p.stdout, tag(12))

    def test_marker_for_another_train_only_is_rejected(self):
        # A 26 sign-off says nothing about 25.10: those boxes keep r11.
        rels = [release(tag(12), trains=["26"]), release(R11)]
        p = run_selection(rels, *STABLE)
        self.assertEqual(p.returncode, 0, p.stderr)
        self.assertEqual(p.stdout, R11)

    def test_grandfathered_full_release_is_approved_for_every_train(self):
        rels = [release(R11), release("v2026.08.19-r10", prerelease=True)]
        for version, train in (STABLE, BETA, ("25.04.2.6", "25.04")):
            p = run_selection(rels, version, train)
            self.assertEqual(p.returncode, 0, p.stderr)
            self.assertEqual(p.stdout, R11, train)

    def test_prerelease_without_marker_is_rejected(self):
        rels = [release(tag(12), prerelease=True), release(R11)]
    ...
    def test_prerelease_with_marker_is_approved(self):
        ...
        rels = [release(tag(12), prerelease=True, trains=["26"]), release(R11)]
        p = run_selection(rels, *BETA)
        self.assertEqual(p.stdout, tag(12))

    def test_draft_is_rejected(self):
        rels = [release(tag(13), draft=True, trains=["25.10"]),
                release(tag(12), draft=True), release(R11)]
        p = run_selection(rels, *STABLE)
        self.assertEqual(p.returncode, 0, p.stderr)
        self.assertEqual(p.stdout, R11)

    def test_newest_approved_release_wins(self):
        rels = [release(tag(12), trains=["25.10"]), release(tag(14), trains=["25.10"]),
                release(tag(13), trains=["25.10"]), release(tag(15), prerelease=True)]
        p = run_selection(rels, *STABLE)
        self.assertEqual(p.stdout, tag(14))

    def test_newest_is_by_publication_date_not_list_order(self):
        rels = [release(R11, published="2026-09-19T00:00:00Z"),
                release(tag(12), trains=["25.10"], published="2026-09-20T00:00:00Z")]
    ...
    def test_marker_must_start_a_line(self):
        ...
        rels = [release(tag(12), prerelease=True, body=body), release(R11)]
        p = run_selection(rels, *STABLE)
        self.assertEqual(p.stdout, R11)

    def test_marker_spacing_and_crlf_are_tolerated(self):
        body = "notes\r\n\r\n<!--verified-train:26-->\r\n"
        rels = [release(tag(12), body=body), release(R11)]
        p = run_selection(rels, *BETA)
        self.assertEqual(p.stdout, tag(12))

    def test_marker_does_not_match_a_longer_train_key(self):
        rels = [release(tag(12), trains=["25.1"]), release(R11)]
        p = run_selection(rels, *STABLE)
        self.assertEqual(p.stdout, R11)
...
class NoCandidate(unittest.TestCase):
    def test_no_approved_release_fails_clearly(self):
        rels = [release(tag(12), prerelease=True), release(tag(13), prerelease=True),
                release(tag(14), trains=["25.10"])]
        p = run_selection(rels, *BETA)
        self.assertNotEqual(p.returncode, 0)
        ...
        self.assertLess(p.stderr.index(tag(14)), p.stderr.index(tag(12)))
    ...
    def test_stable_box_is_pointed_at_stable_hardware_tests(self):
        p = run_selection([release(tag(12), prerelease=True)], *STABLE)
        self.assertNotEqual(p.returncode, 0)
...
"""
            p = run_block(stub)
            self.assertEqual(p.returncode, 0, p.stderr)
            self.assertEqual(p.stdout.strip(), R11)
            self.assertEqual(Path(d, "calls").read_text().split(), ["1", "2"])
            self.assertIn(f"TrueNAS 25.10.7 (train 25.10): newest approved release is {R11}",
                          p.stderr)
