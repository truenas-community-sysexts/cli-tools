"""Run a workflow step's github-script body under node, or its run: block
under bash.

The `script: |` (or `run: |`) block of the named step is extracted verbatim
(dedented from its YAML indentation). For github-script it is wrapped in a
harness that provides the stub `github`, `context` and `core` objects
actions/github-script injects, so the tests execute exactly the code the
workflow runs. node runs from the repo root, where the scripts read
...
OWNER, REPO = "truenas-community-sysexts", "cli-tools"
