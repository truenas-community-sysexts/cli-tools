...
from release_fixtures import R11, issue, marker, release, tag
...
class Approval(unittest.TestCase):
    def test_first_approval_flips_prerelease_adds_marker_and_changelog(self):
        ...
        out = close(issue(tag(12), **STABLE), rels)
    ...
    def test_marker_is_in_the_same_update_as_the_flip(self):
        ...
        rels = [release(tag(12), prerelease=True), release(R11)]
        out = close(issue(tag(12), **PREVIEW), rels)
    ...
    def test_second_train_only_adds_its_marker(self):
        rels = [release(tag(12), prerelease=True), release(R11)]
        first = close(issue(tag(12), **PREVIEW), rels)
        rels = apply(rels, first["updates"][0])
        second = close(issue(tag(12), number=2, **STABLE), rels)
    ...
    def test_marker_is_added_once(self):
        rels = [release(tag(12), prerelease=True), release(R11)]
        first = close(issue(tag(12), **STABLE), rels)
        rels = apply(rels, first["updates"][0])
        again = close(issue(tag(12), number=3, **STABLE), rels)
    ...
    def test_grandfathered_release_is_left_alone(self):
        ...
        # down to this one.
        rels = [release(R11)]
        out = close(issue(R11, **PREVIEW), rels)
    ...
    def test_malformed_train_marker_changes_nothing(self):
        rels = [release(tag(12), prerelease=True)]
        out = close(issue(tag(12), train="26.0.0-BETA.3", labels=("preview-hardware-test",)), rels)
        self.assertEqual(out["updates"], [])
        self.assertIn("not a TrueNAS train key", out["comments"][0])

    def test_missing_release_is_reported(self):
        out = close(issue(tag(99), **STABLE), [release(R11)])
        self.assertEqual(out["updates"], [])
    ...
    def test_changelog_starts_at_the_previous_approved_release(self):
        # r11 was never approved here (still a prerelease): the range must
        # reach back to r10, the last release approved anywhere.
        rels = [release(tag(12), prerelease=True), release(tag(11), prerelease=True),
                release(tag(10))]
        out = close(issue(tag(12), **STABLE), rels)
        self.assertEqual(out["compared"], [f"{tag(10)}...{tag(12)}"])
        self.assertEqual(out["generated"][0]["previous_tag_name"], tag(10))
...
class Legacy(unittest.TestCase):
    ...
    def test_already_full_release_is_not_touched(self):
        out = close(issue(R11), [release(R11)])
        self.assertEqual(out["updates"], [])

    def test_preview_issue_without_train_marker_changes_nothing(self):
        rels = [release(tag(12), prerelease=True)]
        out = close(issue(tag(12), labels=("preview-hardware-test",)), rels)
        self.assertEqual(out["updates"], [])
...
class MakeLatest(unittest.TestCase):
    ...
    def test_stable_approval_of_the_newest_release_takes_latest(self):
        ml, comment = self.latest(issue(tag(12), **STABLE),
                                  [release(tag(12), prerelease=True), release(R11)])
        self.assertEqual(ml, "true")
        self.assertIn("marked Latest", comment)

    def test_preview_approval_never_takes_latest(self):
        ml, _ = self.latest(issue(tag(12), **PREVIEW),
                            [release(tag(12), prerelease=True), release(R11)])
        self.assertEqual(ml, "false")

    def test_older_stable_approval_does_not_regress_latest(self):
        ml, comment = self.latest(issue(tag(12), **STABLE),
                                  [release(tag(13), trains=["25.10"]),
                                   release(tag(12), prerelease=True)])
        self.assertEqual(ml, "false")
        self.assertIn(f"Latest stays on the newer `{tag(13)}`", comment)

    def test_release_approved_on_preview_only_cannot_hold_latest(self):
        ml, _ = self.latest(issue(tag(12), **STABLE),
                            [release(tag(13), trains=["26"]),
                             release(tag(12), prerelease=True), release(R11)])
        self.assertEqual(ml, "true")

    def test_grandfathered_newer_release_keeps_latest(self):
        ml, _ = self.latest(issue(tag(10), **STABLE),
                            [release(R11), release(tag(10), prerelease=True)])
        self.assertEqual(ml, "false")

    def test_preview_approval_keeps_latest_on_a_stable_approved_newest(self):
        ml, _ = self.latest(issue(tag(12), **PREVIEW),
                            [release(tag(12), trains=["25.10"]), release(R11)])
        self.assertEqual(ml, "true")
