...
# cli_tools_init_script_lookup
#
# Locate any registered TrueNAS init script related to this sysext (matches
# "cli-tools-preinit" or ".config/cli-tools" in the command/script field).
# Used by install.sh for --check probing and registration updates, and by
# restore.sh for deregistration.
#
# Prints:
...
try:
    ...
    for s in scripts:
        ...
        if "cli-tools-preinit" in cmd or ".config/cli-tools" in cmd:
